...
### Runtime client profiles and compatibility boundaries

Antigravity outbound identity is selected by the persisted client profile and generated at request time. The canonical profile IDs are `cli` and `ide`; `open-sse/config/antigravityClient.ts` maps the `agy` provider default to `cli` and the `antigravity` provider default to `ide`. `providerSpecificData.clientProfile` is durable credential metadata, not a client-supplied request override. Profile-specific User-Agent and metadata construction lives in `open-sse/services/antigravityHeaders.ts` (`antigravityCliUserAgent()`, `antigravityIdeUserAgent()`, `antigravityIdeNodeUserAgent()`, and `getAntigravityLoadCodeAssistMetadata()`), with product versions resolved by `open-sse/services/antigravityVersion.ts`. Do not copy a fixed UA version from this guide as a canonical value.

The generic `CLI_COMPAT_ANTIGRAVITY` and `CLI_COMPAT_ALL` switches are opt-in ordering controls for the existing fingerprint serializer. They do not select `cli` or `ide`, replace native profile headers, or change provider egress. The current compatibility contracts are synthetic-only and unverified until an operator-approved capture supplies a baseline. This compatibility layer is for supported, operator-controlled account integrations; it is not a claim of official-client identity, and it does not bypass geo, eligibility, quota, subscription, or anti-abuse enforcement.
...
Per-provider table that defines header ordering and JSON body field ordering for compatibility serialization. Currently registered: `codex`, `claude`, `antigravity`, and `github`. Antigravity's native profile identity remains owned by `open-sse/services/antigravityHeaders.ts`; the generic fingerprint entry must not be treated as an approved capture or as a replacement for the `cli`/`ide` profile contract.
...
| `ANTIGRAVITY_USER_AGENT` | Runtime-generated by `open-sse/services/antigravityHeaders.ts` |
...
| `CLI_COMPAT_ANTIGRAVITY=1` | Opt-in Antigravity header/body ordering; native profile identity remains runtime-generated |
