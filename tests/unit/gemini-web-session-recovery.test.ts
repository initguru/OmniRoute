...
describe("recoverGeminiWebSessionWithBrowser (Tier 2 self-healing)", () => {
  ...
  it("falls back to waiting for composer selector if WIZ_global_data is initially missing", async () => {
    ...
    assert.equal(mockPage.waitForSelector.mock.callCount(), 2);
  });

  it("maps __Host- cookies to url: 'https://gemini.google.com' without domain in addCookies", async () => {
    const mockPage = {
      goto: mock.fn(async () => {}),
      url: mock.fn(() => "https://gemini.google.com/app"),
      evaluate: mock.fn(async () => ({
    ...
    let addedCookiesArg: Array<Record<string, unknown>> = [];
    const mockContext = {
      addCookies: mock.fn(async (cookies: Array<Record<string, unknown>>) => {
        addedCookiesArg = cookies;
      }),
      newPage: mock.fn(async () => mockPage),
      cookies: mock.fn(async () => []),
      close: mock.fn(async () => {}),
    ...
    const cookie = "__Host-GAPS=host-cookie-val; __Secure-1PSID=standard-psid-val";
    const result = await recoverGeminiWebSessionWithBrowser({
      cookie,
      playwright: mockPlaywright,
    });
    ...
    assert.equal(result.success, true);
    assert.equal(addedCookiesArg.length, 2);

    const hostCookie = addedCookiesArg.find((c) => c.name === "__Host-GAPS");
    assert.ok(hostCookie, "__Host-GAPS cookie must be present");
    assert.equal(hostCookie.url, "https://gemini.google.com");
    assert.equal(hostCookie.domain, undefined, "__Host- cookie must NOT have domain set");
    assert.equal(hostCookie.secure, true);
    assert.equal(hostCookie.path, "/");

    const standardCookie = addedCookiesArg.find((c) => c.name === "__Secure-1PSID");
    assert.ok(standardCookie, "__Secure-1PSID cookie must be present");
    assert.equal(standardCookie.domain, ".google.com");
    assert.equal(standardCookie.secure, true);
    assert.equal(standardCookie.path, "/");
  });

  it("waits for composer or sign-in selector before evaluating login status to prevent early abort", async () => {
    let waitSelectorArg = "";
    let waitTimeout = 0;
    const mockPage = {
      goto: mock.fn(async () => {}),
      url: mock.fn(() => "https://gemini.google.com/app"),
      waitForSelector: mock.fn(async (selector: string, opts?: { timeout?: number }) => {
        waitSelectorArg = selector;
        waitTimeout = opts?.timeout ?? 0;
      }),
      evaluate: mock.fn(async () => ({
        SNlM0e: "at-token",
        FdrFJe: "fsid-token",
    ...
    const result = await recoverGeminiWebSessionWithBrowser({
      cookie: "__Secure-1PSID=valid-psid",
      playwright: mockPlaywright,
    });

    assert.equal(result.success, true);
    assert.ok(
      waitSelectorArg.includes(".ql-editor") &&
        waitSelectorArg.includes("ServiceLogin") &&
        waitSelectorArg.includes("button[aria-label*='Sign in']"),
