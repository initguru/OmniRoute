...
export function geminiToClaudeResponse(chunk, state) {
  ...
  if (usageMeta && typeof usageMeta === "object") {
    ...
    // In Claude Messages API usage accounting, prompt tokens are broken down into:
    // total prompt tokens = input_tokens (fresh) + cache_read_input_tokens + cache_creation_input_tokens.
    // Gemini's promptTokenCount includes cachedContentTokenCount.
    // Deduct cachedTokens so input_tokens represents fresh prompt tokens and avoids double-counting.
    const inputTokens = Math.max(0, rawPromptTokens - cachedTokens);
