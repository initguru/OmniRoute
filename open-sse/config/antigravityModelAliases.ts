export const ANTIGRAVITY_PUBLIC_MODELS = Object.freeze([
  ...
  // Gemini 3.8 Flash tiers. Dynamic tiered routing resolves high/medium/low
  // to gemini-3.8-flash-tiered upstream.
  {
...
const DYNAMIC_FLASH_TIER_PATTERN =
  /^(?:agy\/|antigravity\/)?gemini-(\d+(?:\.\d+)?)-flash-(high|medium|low)$/;

export function resolveAntigravityModelId(modelId: string): string {
  if (!modelId) return modelId;
  const alias = (ANTIGRAVITY_MODEL_ALIASES as AntigravityModelAliasMap)[modelId];
  if (alias) return alias;
  const match = modelId.match(DYNAMIC_FLASH_TIER_PATTERN);
  if (match) {
    return `gemini-${match[1]}-flash-tiered`;
  }
  return modelId;
}
